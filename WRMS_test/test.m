...
% I/U amplitude ratio:
s.ab_rat = 1;logspace(log10(0.01),log10(1.0),10);
s.ab_rat_rnd = 1e-3;
% I-U pahse shift:
s.ab_phi = 0;[linspace(0,0.5,20) linspace(0.51,0.99,20)]*pi;
s.ab_phi_rnd = pi;
% periods of fundamental harmonic:
s.f0_per = 10;[3,4,5,6,7,9,11,15,20,50,100];
% sampling rate ratio to f0 ratio:
s.fs_rat = 100;logspace(log10(7),log10(100),10);
% spurr amplitude:
s.s_amp = 0.001;logspace(log10(0.01),log10(1.0),10);
% spurr freq to fs/2 ratio:
s.s_freq = 0.005;logspace(log10(0.001),log10(0.9),10);
% spurr analysis mode?
s.spurr_mode = 1;
% bits count:
s.a_bits = 32;
